...
function authConfig(): IAuth {
    ...
    const default_org = process.env.AUTH_DEFAULT_ORG || '';
    const default_users = process.env.AUTH_DEFAULT_USERS || '';
    let session_file = process.env.AUTH_SESSION_FILE || '';
    if (session_file) {
        session_file = Path.join(process.cwd(), session_file);
    }
    return {
        default_strategy, token_name,
        default_org, default_users,
        session_file,
    };
}
